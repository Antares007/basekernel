...
int graphics_write(struct graphics *g, struct graphics_command *command) {
  ...
  while (command && command->type) {
    ...
    case GRAPHICS_COLOR:
      ...
      break;
    case GRAPHICS_RECT:
      graphics_rect(g, command->args[0], command->args[1], command->args[2],
                    command->args[3]);
      break;
    case GRAPHICS_CLEAR:
      graphics_clear(g, command->args[0], command->args[1], command->args[2],
                     command->args[3]);
      break;
    case GRAPHICS_LINE:
      graphics_line(g, command->args[0], command->args[1], command->args[2],
                    command->args[3]);
      break;
    case GRAPHICS_TEXT:
...
uint32_t graphics_width(struct graphics *g) { return g->clip.w; }

uint32_t graphics_height(struct graphics *g) { return g->clip.h; }
...
int graphics_clip(struct graphics *g, int x, int y, int w, int h) {
  ...
  if (x >= g->bitmap->width || y >= g->bitmap->width)
    return 0;

  // Child width must fall within parent size
  if ((x + w) >= g->bitmap->width || (y + h) >= g->bitmap->height)
    return 0;
...
static inline void plot_pixel(struct bitmap *b, int x, int y,
                              struct graphics_color c) {
...
static inline void graphics_line_vert(struct graphics *g, int x, int y, int w,
                                      int h) {
  do {
...
static inline void graphics_line_q1(struct graphics *g, int x, int y, int w,
                                    int h) {
...
static inline void graphics_line_q2(struct graphics *g, int x, int y, int w,
                                    int h) {
...
static inline void graphics_line_q3(struct graphics *g, int x, int y, int w,
                                    int h) {
  int slope = -FACTOR * h / w;
  int counter = 0;
...
static inline void graphics_line_q4(struct graphics *g, int x, int y, int w,
                                    int h) {
  int slope = -FACTOR * w / h;
  int counter = 0;
...
static inline void graphics_line_hozo(struct graphics *g, int x, int y, int w,
                                      int h) {
  do {
    plot_pixel(g->bitmap, x, y, g->fgcolor);
    x++;
...
void graphics_scrollup(struct graphics *g, int x, int y, int w, int h, int dy) {
  ...
  for (j = 0; j < (h - dy); j++) {
    memcpy(&g->bitmap->data[((y + j) * g->bitmap->width + x) * 3],
           &g->bitmap->data[((y + j + dy) * g->bitmap->width + x) * 3], w * 3);
  }
