...
struct list grave_watcher_list = {
    0, 0}; // parent processes are put here to wait for their children
...
void process_kstack_copy(struct process *parent, struct process *child) {
  ...
  struct x86_stack *parent_regs =
      (struct x86_stack *)(parent->kstack_top - sizeof(struct x86_stack));
...
void process_selective_inherit(struct process *parent, struct process *child,
                               int *fds, int fd_len) {
  /* Copy kernel objects */
  int i;
...
int process_data_size_set(struct process *p, unsigned size) {
  ...
  if (size > p->vm_data_size) {
    ...
    pagetable_alloc(p->pagetable, start, size,
...
int process_stack_size_set(struct process *p, unsigned size) {
  ...
  if (size > p->vm_stack_size) {
    uint32_t start = -size;
    pagetable_alloc(p->pagetable, start, size - p->vm_stack_size,
                    PAGE_FLAG_USER | PAGE_FLAG_READWRITE | PAGE_FLAG_CLEAR);
  } else {
...
void process_yield() { process_switch(PROCESS_STATE_READY); }

void process_exit(int code) {
  // printf("process %d exiting with status %d...\n", current->pid, code); -->
  // transport to kshell run
  current->exitcode = code;
  current->exitreason = PROCESS_EXIT_NORMAL;
  process_wakeup_parent(
      &grave_watcher_list); // On exit, wake up parent if need be
...
void process_wakeup_parent(struct list *q) {
  ...
  while (p) {
    if (p->pid == current->ppid && (p->waiting_for_child_pid == 0 ||
                                    p->waiting_for_child_pid == current->pid)) {
      p->state = PROCESS_STATE_READY;
