...
void run_stats(struct stat_args *args) {
  ...
  if (args->stat_type == BCACHE_LIVE) {
    ...
    create_graph(args->stat_type, args->stat_name, args->pid_s, window_width,
                 window_height, plot_width, plot_height, thickness,
                 char_offset);
  ...
  while (1) {
    ...
    max = 1;
    for (i = 0; i < POINTS; i++) {
      if (most_recent_vals[i] > max)
        max = most_recent_vals[i];
    }

    /* Replot points */
    plot_bars(most_recent_vals, max, window_width, window_height, plot_width,
              plot_height, thickness, char_offset);
...
void create_graph(STAT_LIVE stat_type, char *stat_name, char *stat_arg,
  ...
  for (i = 0; i < 10 + 1; i++) {
    draw_rect(x_offset + i * x_tick_width, y_offset + plot_height, thickness,
              6);
  }
  ...
  for (i = 0; i < 10 + 1; i++) {
    draw_rect(x_offset - 4, y_offset + plot_height - i * y_tick_width, 6,
              thickness);
  }
...
void plot_bars(int most_recent_vals[POINTS], int max, int window_width,
  ...
  for (i = 0; i < POINTS; i++) {
    ...
    draw_rect(current_point[0], current_point[1], thickness,
