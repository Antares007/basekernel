...
static struct fs_dirent *fs_dirent_lookup(struct fs_dirent *d,
                                          const char *name) {
  const struct fs_ops *ops = d->volume->fs->ops;
...
struct fs_dirent *fs_dirent_traverse(struct fs_dirent *parent,
                                     const char *path) {
  if (!parent || !path)
    return 0;
...
int fs_dirent_size(struct fs_dirent *d) { return d->size; }

int fs_dirent_isdir(struct fs_dirent *d) { return d->isdir; }
