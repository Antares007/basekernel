...
int sys_process_wrun(const char *path, int argc, const char **argv, int *fds,
                     int fd_len) {
  if (!is_valid_path(path))
    return KERROR_INVALID_PATH;
...
int sys_process_self() { return current->pid; }

int sys_process_parent() { return current->ppid; }

int sys_process_kill(int pid) { return process_kill(pid); }

int sys_process_reap(int pid) { return process_reap(pid); }

int sys_process_wait(struct process_info *info, int timeout) {
  if (!is_valid_pointer(info, sizeof(*info)))
    return KERROR_INVALID_ADDRESS;
...
int sys_process_stats(struct process_stats *s, int pid) {
  if (!is_valid_pointer(s, sizeof(*s)))
    return KERROR_INVALID_ADDRESS;
...
int sys_object_list(int fd, char *buffer, int length) {
  ...
  if (!is_valid_pointer(buffer, length))
    return KERROR_INVALID_ADDRESS;
  if (kobject_get_type(current->ktable[fd]) != KOBJECT_DIR)
    return KERROR_NOT_A_DIRECTORY;
  return kobject_read(current->ktable[fd], buffer, length);
}

int sys_open_file_relative(int fd, const char *path, int mode,
                           kernel_flags_t flags) {
  if (!is_valid_object(fd))
    return KERROR_INVALID_OBJECT;
  if (!is_valid_path(path))
    return KERROR_INVALID_PATH;
...
int sys_open_console(int wd) {
  if (!is_valid_object_type(wd, KOBJECT_GRAPHICS))
    return KERROR_INVALID_OBJECT;
  ...
  current->ktable[fd] =
      kobject_create_console_from_graphics(current->ktable[wd]);
  return fd;
}
...
int sys_object_dup(int fd1, int fd2) {
  if (!is_valid_object(fd1))
    return KERROR_INVALID_OBJECT;
  if (fd2 > PROCESS_MAX_OBJECTS)
    return KERROR_INVALID_OBJECT;
...
int sys_object_size(int fd, int *dims, int n) {
  if (!is_valid_object(fd))
    return KERROR_INVALID_OBJECT;
  if (!is_valid_pointer(dims, sizeof(*dims) * n))
    return KERROR_INVALID_ADDRESS;
...
int sys_object_copy(int src, int dst) {
  if (!is_valid_object(src))
    return KERROR_INVALID_OBJECT;
  if (!is_valid_object(dst))
    return KERROR_INVALID_OBJECT;
...
int sys_system_time(uint32_t *tm) {
  if (!is_valid_pointer(tm, sizeof(*tm)))
    return KERROR_INVALID_ADDRESS;
  struct rtc_time t;
  rtc_read(&t);
...
int sys_system_rtc(struct rtc_time *t) {
  if (!is_valid_pointer(t, sizeof(*t)))
    return KERROR_INVALID_ADDRESS;
  rtc_read(t);
  return 0;
}

int sys_device_driver_stats(const char *name,
                            struct device_driver_stats *stats) {
  if (!is_valid_string(name))
    return KERROR_INVALID_ADDRESS;
  if (!is_valid_pointer(stats, sizeof(*stats)))
    return KERROR_INVALID_ADDRESS;
...
int32_t syscall_handler(syscall_t n, uint32_t a, uint32_t b, uint32_t c,
