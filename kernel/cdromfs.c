...
static struct fs_dirent *cdrom_dirent_create(struct fs_volume *volume,
...
static int cdrom_dirent_read_block(struct fs_dirent *d, char *buffer,
                                   uint32_t blocknum) {
  int nblocks =
      bcache_read(d->volume->device, buffer, 1, d->cdrom.sector + blocknum);
  if (nblocks == 1) {
...
static struct fs_dirent *cdrom_dirent_lookup(struct fs_dirent *dir,
                                             const char *name) {
  if (!dir->isdir)
    return 0;
  ...
  int nsectors =
      dir->size / CDROMFS_BLOCK_SIZE + (dir->size % CDROMFS_BLOCK_SIZE ? 1 : 0);
  ...
  for (i = 0; i < nsectors; i++) {
    ...
    struct iso_9660_directory_entry *d =
        (struct iso_9660_directory_entry *)temp;
    ...
    while (d->descriptor_length > 0) {
      ...
      if (!strncmp(name, dname, dname_length)) {
        struct fs_dirent *r;
        r = cdrom_dirent_create(dir->volume, d->first_sector_little,
                                d->length_little,
...
static int cdrom_dirent_close(struct fs_dirent *d) { return 0; }

static int cdrom_dirent_list(struct fs_dirent *dir, char *buffer,
                             int buffer_length) {
  if (!dir->isdir)
...
static int cdrom_volume_close(struct fs_volume *v) { return 0; }
...
static struct fs cdrom_fs = {"cdromfs", &cdrom_ops, 0};
