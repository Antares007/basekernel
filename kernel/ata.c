...
static int ata_wait(int id, int mask, int state) {
  ...
  int timeout_millis =
      identify_in_progress ? ATA_IDENTIFY_TIMEOUT : ATA_TIMEOUT;
...
static int ata_identify(int id, int command, void *buffer) {
  ...
  if (ata_begin(id, command, 0, 0) &&
      ata_wait(id, ATA_STATUS_DRQ, ATA_STATUS_DRQ)) {
...
static int ata_probe_internal(int id, int kind, int *nblocks, int *blocksize,
                              char *name) {
  uint16_t buffer[256];
  ...
  printf("%s unit %d: %s %u sectors %u MB %s\n",
         (*blocksize) == 512 ? "ata" : "atapi", id,
         (*blocksize) == 512 ? "disk" : "cdrom", *nblocks, mbytes, name);
  return 1;
}
...
int atapi_probe(int id, int *nblocks, int *blocksize, char *name) {
  return ata_probe_internal(id, ATAPI_COMMAND_IDENTIFY, nblocks, blocksize,
                            name);
}

static struct device_driver ata_driver = {.name = "ata",
                                          .probe = ata_probe,
                                          .read = ata_read,
