...
int elf_load(struct process *p, const char *filename, addr_t *entry) {
  ...
  if (strncmp(header.ident, "\177ELF", 4) ||
      header.machine != ELF_HEADER_MACHINE_I386 ||
      header.version != ELF_HEADER_VERSION)
    goto noexec;

  actual = fs_dirent_read(d, (char *)&program, sizeof(program),
                          header.program_offset);
  if (actual != sizeof(program))
    goto noload;

  // printf("elf: text %x bytes from offset %x at address %x length
  // %x\n",program.file_size,program.offset,program.vaddr,program.memory_size);

  if (program.type != ELF_PROGRAM_TYPE_LOADABLE ||
      program.vaddr < PROCESS_ENTRY_POINT || program.memory_size > 0x8000000 ||
      program.memory_size != program.file_size)
    goto noexec;

  process_data_size_set(p, program.memory_size);

  actual = fs_dirent_read(d, (char *)program.vaddr, program.memory_size,
                          program.offset);
  if (actual != program.memory_size)
    goto mustdie;

  for (i = 0; i < header.shnum; i++) {
    actual = fs_dirent_read(d, (char *)&section, sizeof(section),
                            header.section_offset + i * header.shentsize);
    ...
    if (section.type == ELF_SECTION_TYPE_BSS) {
      ...
      if (limit > p->vm_data_size) {
        process_data_size_set(p, section.address + section.size -
                                     PROCESS_ENTRY_POINT);
