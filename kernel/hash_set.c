...
static unsigned hash_set_list_add(struct hash_set_node **head,
...
static struct hash_set_node *hash_set_list_lookup(struct hash_set_node *head,
                                                  unsigned key) {
...
static unsigned hash_set_list_delete(struct hash_set_node **head,
                                     unsigned key) {
  struct hash_set_node *prev = 0, *curr = *head;
  while (curr && (curr->key < key)) {
    prev = curr;
...
struct hash_set *hash_set_create(unsigned buckets) {
  struct hash_set_node **set_nodes =
      kmalloc(sizeof(struct hash_set_node *) * buckets);
...
unsigned hash_set_entries(struct hash_set *set) { return set->num_entries; }
