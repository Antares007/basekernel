...
struct kobject *kobject_create_graphics_from_graphics(struct kobject *k, int x,
                                                      int y, int w, int h) {
  if (k->type != KOBJECT_GRAPHICS)
    return 0;
...
struct kobject *kobject_create_file_from_dir(struct kobject *kobject,
                                             const char *name) {
...
struct kobject *kobject_create_dir_from_dir(struct kobject *kobject,
                                            const char *name) {
  if (kobject->type == KOBJECT_DIR) {
...
int kobject_read(struct kobject *kobject, void *buffer, int size) {
  ...
  switch (kobject->type) {
  case KOBJECT_FILE:
    actual = fs_dirent_read(kobject->data.file, (char *)buffer, (uint32_t)size,
                            kobject->offset);
    break;
  case KOBJECT_DIR:
    ...
    break;
  case KOBJECT_DEVICE:
    actual = device_read(kobject->data.device, buffer,
                         size / device_block_size(kobject->data.device), 0);
    break;
  case KOBJECT_PIPE:
...
int kobject_read_nonblock(struct kobject *kobject, void *buffer, int size) {
  switch (kobject->type) {
  case KOBJECT_DEVICE:
    return device_read_nonblock(kobject->data.device, buffer,
                                size / device_block_size(kobject->data.device),
                                0);
  case KOBJECT_PIPE:
...
int kobject_write(struct kobject *kobject, void *buffer, int size) {
  ...
  case KOBJECT_GRAPHICS:
    return graphics_write(kobject->data.graphics,
                          (struct graphics_command *)buffer);
  case KOBJECT_CONSOLE:
    return console_write(kobject->data.console, buffer, size);
  case KOBJECT_FILE: {
    int actual = fs_dirent_write(kobject->data.file, (char *)buffer,
                                 (uint32_t)size, kobject->offset);
    if (actual > 0)
      kobject->offset += actual;
    return actual;
  }
  case KOBJECT_DEVICE:
    return device_write(kobject->data.device, buffer,
                        size / device_block_size(kobject->data.device), 0);
  case KOBJECT_PIPE:
...
int kobject_lookup(struct kobject *kobject, const char *name,
...
int kobject_get_type(struct kobject *kobject) { return kobject->type; }
