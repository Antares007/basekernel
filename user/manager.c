...
int main(int argc, char *argv[]) {
      ...
      if (current_pos[row][0] + programs[p_i].w + 4 * padding <= std_dims[0]) {
        ...
        if (current_pos[row][0] == 0) {
          ...
          if (current_pos[row][1] + programs[p_i].h + 4 * padding >
            ...
            current_pos[row + 1][1] =
                current_pos[row][1] + spacing + programs[p_i].h + 4 * padding;
          }
        }
        ...
        current_pos[row][0] =
            current_pos[row][0] + spacing + programs[p_i].w + 4 * padding;
        ;
        break;
      }
    }
  ...
  for (p_i = 0; p_i < num_programs; ++p_i) {
    ...
    fds[p_i][3] =
        syscall_open_window(KNO_STDWIN, placement[p_i][0], placement[p_i][1],
                            programs[p_i].w, programs[p_i].h);
    ...
    draw_border(placement[p_i][0] - 2 * padding,
                placement[p_i][1] - 2 * padding, programs[p_i].w + 4 * padding,
  ...
  draw_border(placement[p_act][0] - 2 * padding,
              ...
              programs[p_act].w + 4 * padding, programs[p_act].h + 4 * padding,
              padding, 0, 0, 255);
  draw_flush();
  ...
  while (tin != '~') {
    ...
    if (tin == '\t') {
      ...
      draw_border(placement[p_act][0] - 2 * padding,
                  ...
                  programs[p_act].h + 4 * padding, padding, 255, 255, 255);
      draw_flush();
      p_act = (p_act + 1) % num_programs;
      ...
      draw_border(placement[p_act][0] - 2 * padding,
                  placement[p_act][1] - 2 * padding,
                  programs[p_act].w + 4 * padding,
                  programs[p_act].h + 4 * padding, padding, 0, 0, 255);
      draw_flush();
