...
static struct device *device_create(struct device_driver *dd, int unit,
                                    int nblocks, int block_size) {
...
int device_read(struct device *d, void *data, int size, int offset) {
  int status;
  if (d->driver->read) {
    status = d->driver->read(d->unit, data, size * d->multiplier,
                             offset * d->multiplier);
    if (status) {
...
int device_read_nonblock(struct device *d, void *data, int size, int offset) {
  ...
  if (d->driver->read_nonblock) {
    status = d->driver->read_nonblock(d->unit, data, size * d->multiplier,
                                      offset * d->multiplier);
    if (status) {
      d->driver->stats.blocks_read += size * d->multiplier; // number of blocks
    }
...
int device_write(struct device *d, const void *data, int size, int offset) {
  int status;
  if (d->driver->write) {
    status = d->driver->write(d->unit, data, size * d->multiplier,
                              offset * d->multiplier);
    if (!status) {
...
int device_nblocks(struct device *d) { return d->nblocks / d->multiplier; }

int device_unit(struct device *d) { return d->unit; }

const char *device_name(struct device *d) { return d->driver->name; }
